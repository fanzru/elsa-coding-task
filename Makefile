...
PG_PORT      ?= 5439
DATABASE_URL ?= postgres://quiz:quiz@127.0.0.1:$(PG_PORT)/quiz_test
WEB_PORT     ?= 3000
SERVER_PORT  ?= 4000
CLIENTS      ?= 1000
WORKERS      ?= 8
...
help: ## Show this help
	...
	@printf "\nVariables: WEB_PORT=$(WEB_PORT) SERVER_PORT=$(SERVER_PORT) CLIENTS=$(CLIENTS) WORKERS=$(WORKERS) REDIS_URL=$(REDIS_URL)\n\n"
...
##@ Run
dev: ## Run server (:4000) and web (:3000) with live reload; WEB_PORT/SERVER_PORT to change
	PORT=$(SERVER_PORT) pnpm --filter @quiz/server dev & \
	pnpm --filter @quiz/web exec next dev -p $(WEB_PORT); kill %1 2>/dev/null

dev-server: ## Run only the quiz server
	PORT=$(SERVER_PORT) pnpm --filter @quiz/server dev
...
start: build ## Build, then run the production server and web
	PORT=$(SERVER_PORT) NODE_ENV=production node apps/server/dist/index.js & \
	pnpm --filter @quiz/web exec next start -p $(WEB_PORT); kill %1 2>/dev/null

##@ Quality
test: ## Unit, property, actor and integration tests (cluster tests skipped without REDIS_URL)
	pnpm test

test-redis: redis-up ## Test suite including cluster tests against a throwaway Redis
	REDIS_URL=$(REDIS_URL) pnpm test; status=$$?; $(MAKE) redis-down; exit $$status

test-full: redis-up pg-up ## Everything: unit + integration + cluster (Redis) + archive (Postgres)
	REDIS_URL=$(REDIS_URL) DATABASE_URL=$(DATABASE_URL) pnpm test; status=$$?; $(MAKE) redis-down pg-down; exit $$status
...
check: lint typecheck test ## Everything CI runs (without Redis)
...
##@ Database (Postgres)
pg-up: ## Start a throwaway Postgres in Docker (PG_PORT, default 5439; db quiz_test, user/pass quiz)
	@docker ps --format '{{.Names}}' | grep -q '^quiz-postgres$$' || \
	  docker run --rm -d --name quiz-postgres -p 127.0.0.1:$(PG_PORT):5432 \
	    -e POSTGRES_USER=quiz -e POSTGRES_PASSWORD=quiz -e POSTGRES_DB=quiz_test postgres:17-alpine >/dev/null
